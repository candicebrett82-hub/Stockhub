  ...
  try {
    ...
    if (type === "pos" || type === "both" || !type) {
      ...
      for (const row of pos.recordset) {
        ...
        grouped[key].items.push({ sku: row.sku || "", description: row.description || "", qty: row.qty || 1, unitCost: row.unitCost || 0, serial: (row.serial || row.serialAlt || "").trim(), manufacturer: row.manufacturer || "", vendor: row.vendor || "", dateReceived: row.dateReceived, bookedInStatus: row.bookedInStatus || "" });
      }
